...
假如你现在修改了**远程仓库的主邮箱**，比如 GitHub 能绑定多个邮箱，但必须指定其中一个为主邮箱，因为你在配置本地 Git 信息的时候需要配置远程仓库账号的邮箱。虽然还有个用户名配置（不变），但 Git 就是会把你当作一个全新的 “你” ，因此在本地，你拉取的远程仓库的代码中已有的 版本控制记录，只会是属于你之前的邮箱和用户名所代表的“你”，而不属于现在的“你”。

这会造成，你 通过 SSH 拉不下来：提示无访问权限。

我是遇到这个问题，在 `git version 2.25.0.windows.1`
